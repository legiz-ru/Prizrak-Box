...
// Было 4.6 (запас над AA 4.5) — сам запас на светлых обоях выбирал заметно
// больше непрозрачности, чем нужно для минимума AA; целимся ровно в него.
const SCRIM_TARGET_CONTRAST = 4.5;
// Было 0.62. На светлом «горячем пятне» обоев (яркое небо, окно, блик)
// бисекция раньше почти всегда упиралась в потолок — вся панель гасла
// почти до сплошного цвета ради контраста в одном локальном пятне. Ниже
// потолок — картинка остаётся угадываемой даже в этом крайнем случае;
// для всех обычных (не почти-белых) пятен на итоговую альфу это не влияет
// вовсе, потолок в них и раньше не был нужен.
 ...
 * держит 4.6:1 над худшим участком обоев. Ноль означает «этим обоям подложка
 * не нужна» — блок тогда выглядит ровно как сегодня.
 */
