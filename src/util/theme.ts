...
// Было 4.6, потом 4.5 (строгий AA) — на светлых обоях панель всё равно
// читалась заметно темнее самой картинки. Мелкий приглушённый текст не
// обязан держать полный AA как основной контент; спускаемся ниже него в
// пользу того, чтобы обои оставались видимыми.
const SCRIM_TARGET_CONTRAST = 4.0;
// Было 0.62, потом 0.55 — даже обычные (не почти-белые) светлые пятна
// почти всегда упирались в этот потолок. Ниже — подложка ощутимо легче
// на любом обычном светлом фрагменте, а не только в крайнем случае.
