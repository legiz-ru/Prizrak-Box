...
//
// The frontend learns the px control-plane host/port/secret exactly once during
// bootstrap — from the window URL query string under Electron, and from
// window.pxConnInfo (CoreService.GetConnInfo) under the Wails shell — and the
// axios instance is built from those values a single time.
//
...
// instance so the running app keeps talking to the right px. Bootstrap applies
// the initial info through the same function.
