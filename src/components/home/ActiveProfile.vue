...
.profile-card {
  ...
  /* Подложка поверх штатной заливки — та же, что несут карточки в разделах
     ...
     ровно как раньше. Скрим её не решает: на тёмных обоях (обычный случай)
     альфа всегда 0, а край карточки терялся именно там — на детальном фото
     даже --sub-card-border (alpha .3) почти не читается. */
  background:
    linear-gradient(rgba(var(--px-scrim-rgb), var(--px-scrim-a)),
    rgba(var(--px-scrim-rgb), var(--px-scrim-a))),
    var(--sub-card-bg);
  /* Более плотный край, отдельный от --sub-card-border: карточка текущего
     профиля — единственный ориентир поверх обоев, ей нужна видимая граница
     независимо от скрима. --right-box-shadow (несимметричная, смещённая
     2px/2px тень) хорошо держала низ и право, но не давала никакой
     подсветки сверху и слева — там край держался только на тонкой рамке.
     --px-elev-1 — тот же симметричный многослойный профиль тени, что несёт
     любая другая обозначенная карточка в приложении. */
