...
Управляет тем, отображается ли переключатель режимов (Rule / Global / Direct) в левом боковом меню.
...
Если значение равно `false` (без учёта регистра — сравнение через `.toLowerCase()`) или `0`, **весь переключатель режимов** (Rule / Global / Direct) скрывается из левого бокового меню для этого профиля. Любое другое значение или отсутствие заголовка оставляет переключатель видимым.
