...
|----------|----------|
| Тип | `string` (URL) |
| Поддерживаемые форматы | PNG, JPEG, WebP, SVG, GIF |
...
|-----------|-------------|----------|
| `X-Hwid` | Обязательный | Аппаратный отпечаток — уникальный идентификатор устройства |
| `X-Device-OS` | Необязательный | Название операционной системы (например, `Windows`, `Android`, `macOS`) |
| `X-Ver-OS` | Необязательный | Версия ОС (например, `14`, `11`, `14.5`) |
| `X-Device-Model` | Необязательный | Модель устройства (например, `Pixel 8`, `iPhone 14 Pro Max`) |
...
Следующие заголовки поддерживаются исключительно **Android-версией** Prizrak-Box (ветка prizrak репозитория ClashMetaForAndroid).
