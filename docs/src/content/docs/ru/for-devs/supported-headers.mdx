...
Управляет тем, доступен ли пункт **Global** в переключателе режимов (Rule / Global / Direct) в левом боковом меню.

| Свойство | Значение |
|----------|----------|
| Тип | `string` |
| Значения | `false` скрывает Global; любое другое значение (или отсутствие) оставляет его |
...
Если значение строго `false` (без учёта регистра — сравнение через `.toLowerCase()`), режим **Global** скрывается/запрещается для этого профиля, оставляя только **Rule** (и **Direct**). Любое другое значение или отсутствие заголовка оставляет **Global** доступным.

<Aside type="note">
Если в момент применения заголовка профиль находится в режиме **Global**, приложение автоматически переключает активный профиль в режим **Rule**.
</Aside>
