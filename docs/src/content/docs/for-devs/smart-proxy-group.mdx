...
LightGBM integration is disabled by default. The weight-based algorithm works without it.
</Aside>
...
- `/etc/openclash/Model.bin` (OpenClash installs)

### Collecting training data

Enable `collectdata: true` to write connection metrics to `smart_weight_data.csv`:
- `~/smart_weight_data.csv` (home directory)
- `/etc/openclash/smart_weight_data.csv` (OpenClash installs)
...
|--------|------|-------------|
| `GET` | `/group/{name}/weights` | Query current node weights for a group |
| `GET` | `/group/weights` | Query weights for all smart groups |
| `POST` | `/cache/smart/flush` | Flush all smart group caches |
| `POST` | `/cache/smart/flush/{config}` | Flush cache for a specific config |
| `DELETE` | `/connections/smart/{id}` | Block a connection and force re-selection |
| `POST` | `/upgrade/lgbm` | Download and apply the latest LightGBM model |
