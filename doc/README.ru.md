...
- `url` (обязательный): URL подписки для импорта
...
2. **Импорт от разных провайдеров:**
   ```
...
Deeplink поддерживает все типы контента, которые поддерживает ручной импорт профилей:

- URL подписок (HTTP/HTTPS)
- Ссылки для общего доступа (vmess://, vless://, ss://, и т.д.)
- Конфигурации в кодировке Base64
- Конфигурации YAML
- Конфигурации JSON

### Использование

1. Пользователь кликает на deeplink с веб-страницы или из приложения
2. Операционная система запускает Prizrak-Box (или переводит его в фокус)
3. Приложение автоматически импортирует профиль
4. Пользователь получает уведомление об успехе/ошибке
5. Новый профиль появляется в списке профилей
