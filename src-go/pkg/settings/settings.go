...
func loadSettings() {
	...
	if err != nil {
		...
		log.Printf("Settings file not found or unreadable, using defaults: %v", err)
		return
	}
	...
	if err := json.Unmarshal(data, &settings); err != nil {
		...
		log.Printf("Invalid settings JSON, using defaults: %v", err)
		return
	}
	...
	log.Printf("Loaded settings from file: HWID=%v", settings.HWID)
}
...
func SetHWIDSetting(enabled bool) error {
	...
	log.Printf("HWID setting changed from %v to %v", oldValue, enabled)
